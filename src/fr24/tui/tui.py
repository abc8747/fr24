...
class FR24Tui(App[None]):
    ...
    def action_refresh(self) -> None:
        ...
        if aircraft := aircraft_widget.aircraft_id:
            self.run_worker(
                self.lookup_aircraft(aircraft, ts=ts), exclusive=True
            )
            return
        number_widget = self.query_one(FlightWidget)
        if number := number_widget.number:
            self.run_worker(self.lookup_number(number, ts=ts), exclusive=True)
            return
        ...
        if departure := departure_widget.airport_id:
            if arrival := arrival_widget.airport_id:
                ...
                self.run_worker(
                    self.lookup_city_pair(departure, arrival, ts=ts),
                    exclusive=True,
                )
                return
            self.run_worker(
                self.lookup_departure(departure, ts=ts), exclusive=True
            )
            return
        if arrival := arrival_widget.airport_id:
            self.run_worker(self.lookup_arrival(arrival, ts=ts), exclusive=True)
            return
    ...
    async def lookup_arrival(
        ...
        try:
            ...
            results = result.to_dict()
            s = results["result"]["response"]["airport"]["pluginData"][
                "schedule"
            ]
